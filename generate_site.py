...
try:
    ...
    if not service_account_info_str:
        # Эта ошибка теперь будет видна в логах сборки Netlify
    ...
    print("Убедитесь, что переменная окружения FIREBASE_KEY добавлена в настройки Netlify и содержит корректный JSON.")
    exit(1)
...
def generate_product_pages(all_data):
    ...
    for item in products:
        ...
        try:
            # --- НАЧАЛО ИЗМЕНЕНИЯ ---
            # Создаем отфильтрованный список картинок (без YouTube) прямо здесь в Python.
            # Это решает проблему "No test named 'match'" в Jinja2.
            image_urls = [
            ...
            html_content = template.render(
                ...
                image_urls=image_urls  # <-- Передаем новый готовый список в шаблон
            )
            with open(path, 'w', encoding='utf-8') as f:
                f.write(html_content)
            print(f"✓ Создана страница: /product/{slug}/")
        except Exception as e:
            # Эта строка теперь будет ловить ошибку и показывать ее в логах Netlify
