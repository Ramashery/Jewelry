...
def generate_product_pages(all_data):
    ...
    for item in products:
        ...
        try:
            # Готовим отфильтрованный список картинок (без YouTube) для шаблона
            image_urls = [
                url for url in item.get('images', []) 
                if 'youtube.com' not in url and 'youtu.be' not in url
            ]
            
            # Готовим описание специально для JSON, чтобы избежать ошибок с кавычками
            description_for_json = item.get('metaDescription') or item.get('description', '').split('\n')[0]
            ...
            html_content = template.render(
                ...
                description_for_json=description_for_json
            )
