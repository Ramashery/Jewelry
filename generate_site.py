...
def generate_schemas(item, all_categories, image_urls):
    ...
    # !!! ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ: Гарантируем правильные пробелы !!!
    # 1. Заменяем все теги и переносы строк на пробелы
    description_from_db = item.get('description', '')
    text_with_spaces = re.sub('<[^<]+?>', ' ', description_from_db).replace('\n', ' ')
    # 2. "Схлопываем" множественные пробелы в один и убираем пробелы по краям
