...
def generate_site():
    ...
    # --- ИСПРАВЛЕНИЕ: Ищем шаблон сначала в корне, а если нет — в папке en/ ---
    post_template_path = 'post.html'
    if not os.path.exists(post_template_path):
        if os.path.exists('en/post.html'):
            print("  -> INFO: post.html найден в папке 'en/'. Используем его.")
            post_template_path = 'en/post.html'
        else:
            print("  -> ERROR: post.html не найден ни в корне, ни в папке 'en/'")
