...
try:
    ...
    if not service_account_json_str:
        ...
        if os.path.exists('firebase-key.json'):
            ...
            print("ВНИМАНИЕ: Файл 'firebase-key.json' не найден.")
    ...
    if service_account_json_str:
        ...
        # Если ключей нет, скрипт не сможет работать с БД, но попытаемся не ронять сразу,
        # хотя скорее всего упадет дальше.
        db = None
        print("ВНИМАНИЕ: Ключи доступа не найдены.")
...
# --- ВАЖНОЕ ИСПРАВЛЕНИЕ: ОПРЕДЕЛЯЕМ ФИЛЬТР ДО ЗАГРУЗКИ ШАБЛОНА ---
...
# И только теперь загружаем шаблон
try:
    template = env.get_template('template.html')
except Exception as e:
    print(f"Ошибка загрузки template.html: {e}")
    template = None
...
def generate_site():
    ...
    if not db:
        print("Нет подключения к БД, пропускаем генерацию динамических страниц.")
        return
    ...
    # 3. Генерация БЛОГА (Создание папок /en/post/slug/)
    ...
    if os.path.exists(post_template_path):
        ...
        print(f"  -> ОШИБКА: Шаблон поста '{post_template_path}' не найден!")
    ...
    if template:
        ...
        print("  -> Пропуск генерации товаров (шаблон не загружен).")
