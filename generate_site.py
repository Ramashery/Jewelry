...
def generate_schemas(item, all_categories, image_urls):
    ...
    # !!! ИСПРАВЛЕНИЕ 1: Создаем "чистое" описание без HTML и лишних пробелов
    clean_description = item.get('description', '').replace('<p>', '').replace('</p>', '').replace('\n', ' ').strip()
    
    # !!! ИСПРАВЛЕНИЕ 3: Генерируем строку категории
    item_category_id = item.get('category')
    category_info = all_categories.get(item_category_id) if item_category_id else None
    category_string = "Jewelry" # Значение по умолчанию
    if category_info:
    ...
    product_schema = {
        "@type": "Product",
        # !!! ИСПРАВЛЕНИЕ 2: Убираем лишние пробелы с помощью .strip()
        ...
        "url": product_url,
        "category": category_string # !!! ИСПРАВЛЕНИЕ 3: Добавляем поле категории
    }
